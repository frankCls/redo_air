...
public abstract class DataSetPersistenceTest extends PersistenceTest {
		...
		try (Connection jdbcConnection = newJdbcConnection()) {
			...
			if (databaseEngine == DatabaseEngine.mysql) {
				connection.getConfig().setProperty(DatabaseConfig.PROPERTY_DATATYPE_FACTORY,
						new MySqlDataTypeFactory()); // Set factorytype in
														// dbconfig to remove
														// warning
			}
	...
	/**
	 * Callback hook for subclasses to trigger their behaviour before
	 * initializing datasets.
	 */
	protected abstract void initialize() throws Exception;

	/**
	 * Callback hook for subclasses to trigger their behaviour before unloading
	 * datasets.
	 */
	 ...
	 *             When the stars are in the wrong position.
	 */
	...
	/**
	 * Provides a list of datasets that should be loaded. Subclasses can
	 * override this to provide customization.
	 * 
	 * @return An array of Strings pointing to class-path relative dataset XML
	 *         files.
	 */
